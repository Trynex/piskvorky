...
["", "piskvorky.dart", , G, {
  "": "",
  main: [function() {
    var t1, t2, i, t3, x, y, canvas;
    $.get$score().textContent = "Krizik: " + C.JSInt_methods.toString$0($.crossScore) + " - Gulicka: " + C.JSInt_methods.toString$0($.circleScore);
    $.cross = W.ImageElement_ImageElement(null, "./images/krizik.png", null);
    $.circle = W.ImageElement_ImageElement(null, "./images/gulicka.png", null);
    t1 = $.scale;
    t2 = Array(t1);
    $.game = t2;
    for (i = 0; i < t1; ++i) {
    ...
    canvas = document.querySelector("#plocha");
    $.bounce = canvas.getBoundingClientRect();
    t1 = $.squareSize * $.scale + 6;
    t2 = J.getInterceptor$x(canvas);
    t2.set$width(canvas, t1);
    t2.set$height(canvas, t1);
    $.canvas2D = t2.get$context2D(canvas);
    t2 = J.get$onLoad$x($.cross);
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(G.drawGame$closure()), t2._useCapture), [H.getTypeArgumentByIndex(t2, 0)])._tryResume$0();
    t2 = J.get$onLoad$x($.circle);
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(G.drawGame$closure()), t2._useCapture), [H.getTypeArgumentByIndex(t2, 0)])._tryResume$0();
    C.CanvasElement_methods.addEventListener$2(canvas, "click", G.changeSquare$closure());
    C.CanvasElement_methods.addEventListener$2(canvas, "click", G.drawGame$closure());
  }, "call$0", "main$closure", 0, 0, 0],
  drawGame: [function(e) {
    var t1, i, t2, j, t3;
    t1 = $.canvas2D;
    t1.fillStyle = "rgba(180, 180, 180, 0)";
    t1.lineWidth = 1;
    t1.strokeStyle = "#d7d7d7";
    for (i = 0; i <= $.scale; ++i) {
      $.canvas2D.moveTo($.squareSize * i, 0);
      t1 = $.canvas2D;
      t2 = $.squareSize;
      t1.lineTo(t2 * i, t2 * $.scale);
    }
    for (i = 0; t1 = $.scale, t2 = $.canvas2D, i <= t1; ++i) {
      t2.moveTo(0, $.squareSize * i);
      t1 = $.canvas2D;
      t2 = $.squareSize;
      t1.lineTo(t2 * $.scale, t2 * i);
    }
    t2.stroke();
    for (i = 0; i < $.scale; ++i)
      for (j = 0; j < $.scale; ++j) {
  ...
  }, "call$1", "drawGame$closure", 2, 0, 8],
  checkRowsCols: function(color) {
    var t1, t2, t3, i, counter, counter2, j, t4, t5;
    for (t1 = $.scale, t2 = $.length, t3 = $.game, i = 0; i < t1; ++i)
      for (counter = 0, counter2 = 0, j = 0; j < t1; ++j) {
        t4 = t3.length;
        if (i >= t4)
          return H.ioore(t3, i);
        t5 = t3[i];
        if (j >= t5.length)
          return H.ioore(t5, j);
        counter = t5[j] === color ? counter + 1 : 0;
        if (j >= t4)
          return H.ioore(t3, j);
        t4 = t3[j];
        if (i >= t4.length)
          return H.ioore(t4, i);
        counter2 = t4[i] === color ? counter2 + 1 : 0;
        if (counter === t2 || counter2 === t2)
          return true;
      }
    return false;
  },
  diagonalToRight: function(color, a, b) {
    var t1, t2, t3, t4, counter, i, t5, t6, t7;
    for (t1 = $.scale, t2 = t1 - 2, t3 = $.length, t4 = $.game, counter = 0, i = 0; i < t1; ++i) {
      t5 = a + i;
      if (t5 >= t4.length)
      ...
      counter = t6[t7] === color ? counter + 1 : 0;
      if (counter === t3)
        return true;
      else if (t5 > t2 || t7 > t2)
        return false;
  ...
  diagonalToLeft: function(color, a, b) {
    var t1, t2, t3, t4, counter, i, t5, t6, t7;
    for (t1 = $.scale, t2 = $.length, t3 = $.game, t4 = t1 - 2, counter = 0, i = 0; i < t1; ++i) {
      t5 = a - i;
  ...
  checkDiagonal: function(color) {
    var i, t1;
    for (i = 0; i < $.scale; ++i)
      if (G.diagonalToRight(color, i, 0))
        return true;
    for (i = 0; t1 = $.scale, i < t1; ++i)
      if (G.diagonalToRight(color, 0, i))
        return true;
    for (i = t1 - 1; i >= 0; --i)
      if (G.diagonalToLeft(color, i, 0))
        return true;
    for (i = 0; t1 = $.scale, i < t1; ++i)
      if (G.diagonalToLeft(color, t1 - 1, i))
        return true;
    return false;
  },
  changeSquare: [function($event) {
    ...
    if ($.winner)
      ...
      t2 = t1.get$offset($event);
      t2 = t2.get$x(t2);
      t3 = $.squareSize;
      if (typeof t2 !== "number")
        return t2.$div();
      x = C.JSNumber_methods.toInt$0(Math.floor(t2 / t3));
      t1 = t1.get$offset($event);
      t1 = t1.get$y(t1);
      t3 = $.squareSize;
      if (typeof t1 !== "number")
        return t1.$div();
      y = C.JSNumber_methods.toInt$0(Math.floor(t1 / t3));
      t1 = $.game;
      if (x < 0 || x >= t1.length)
        return H.ioore(t1, x);
      t1 = t1[x];
      ...
      if (t1[y] === -1) {
        ...
        if (t2 === 1) {
          t1[y] = 1;
          if (G.checkRowsCols(t2) || G.checkDiagonal(t2)) {
            $.get$message().textContent = "Krizik vyhral. Kliknite pre novu hru.";
            t1 = $.crossScore + 1;
            $.crossScore = t1;
            $.get$score().textContent = "Krizik: " + C.JSInt_methods.toString$0(t1) + " - Gulicka: " + C.JSInt_methods.toString$0($.circleScore);
            $.winner = true;
          } else {
            $.turn = 0;
            $.get$message().textContent = "Pekny tah, ide gulicka.";
          }
        } else {
          t1[y] = 0;
          if (G.checkRowsCols(t2) || G.checkDiagonal(t2)) {
            $.get$message().textContent = "Gulicka vyhrala. Kliknite pre novu hru.";
            $.circleScore = $.circleScore + 1;
            $.get$score().textContent = "Krizik: " + C.JSInt_methods.toString$0($.crossScore) + " - Gulicka: " + C.JSInt_methods.toString$0($.circleScore);
            $.winner = true;
          } else {
            $.turn = 1;
            $.get$message().textContent = "Pekny tah, ide krizik.";
          }
        }
      } else
        $.get$message().textContent = "Prepac, ale toto policko nie je volne.";
    }
  }, "call$1", "changeSquare$closure", 2, 0, 9],
  restartGame: function() {
    var t1, t2, i, j, t3, tah;
    for (t1 = $.scale, t2 = $.game, i = 0; i < t1; ++i)
      for (j = 0; j < t1; ++j) {
        if (i >= t2.length)
          return H.ioore(t2, i);
        t3 = t2[i];
    ...
    $.get$message().textContent = "Nova hra sa zacala. Zacina " + tah + " !";
    t1 = $.canvas2D;
    t1.fillStyle = "rgba(255, 255, 255, 1)";
    t2 = $.squareSize * $.scale;
    t1.clearRect(0, 0, t2, t2);
  }
},
1],
]);
...
Isolate.$lazy($, "score", "score", "get$score", function() {
  return document.querySelector("#score");
});
Isolate.$lazy($, "message", "message", "get$message", function() {
